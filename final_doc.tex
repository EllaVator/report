...
It consists of a set of two simple grammars, one in German and one in English, in the JSGF format\footnote{\url{http://www.w3.org/TR/jsgf/}} that is usable with Sphinx.
...
The English dictionary is based on the CMU Pronouncing Dictionary\footnote{Lenzo, Kevin. \textit{The CMU Pronouncing Dictionary.} \url{http://www.speech.cs.cmu.edu/cgi-bin/cmudict}}, an open-source  machine-readable pronunciation dictionary for North American English.
Words that could not be found in the existing CMU dictionary, which was mainly the case for proper nouns, were entered into the LOGIOS lexicon tool.\footnote{\textit{LOGIOS Lexicon Tool}. \url{http://www.speech.cs.cmu.edu/tools/lextool.html}}


The German dictionary is based on the voxforge German CMU dictionary which can be found via sourceforge.%
\footnote{CMU Sphinx. Speech Recognition Toolkit. Slashdot Media, 2016. \url{http://cmusphinx.sourceforge.net/}}
...
Lists one and two  of the Harvard sentences\footnote{Harvard Sentences. \url{http://www.cs.columbia.edu/~hgs/audio/harvard.html}} were used as phonetically balanced sentences in English and twenty sentences taken from BITS\footnote{\url{http://www.bas.uni-muenchen.de/forschung/Bas/BasBITSUSTABLE}} were used for German.
...
The JSGF uses the style and conventions of Java, as well as those of traditional grammar notations.\footnote{ JSpeech Grammar Format. \textit{W3CNote}. \url{http://www.w3.org/TR/jsgf}}
...
Here are couple (\autoref{fig:serialsetup1}, \autoref{fig:serialsetup2}) of \textbf{incorrect} pin connection screenshots included for historical purposes.\\

% we need a few 0pics here when we finish the setup
\begin{figure}
\center{\includegraphics[width=10.0cm, angle=0]{bad_serial_setup3.JPG}}
\caption{Grey: GROUND, purple: TRANSMIT, blue: RECEIVE}
\label{fig:serialsetup1}
\end{figure}

\begin{figure}
\center{\includegraphics[scale=0.45, angle=0]{bad_serial_setup4.JPG}}
\caption{Grey: GROUND, purple: TRANSMIT, blue: RECEIVE}
...
Serial implementation follows mainly RXTX examples provided online%
\footnote{\url{http://rxtx.qbang.org/wiki/index.php/Two_way_communcation_with_the_serial_port}}
as well as source files of the original project.
...
\footnote{See \url{https://en.wikipedia.org/wiki/Serial_port} for details}\\
Serial port name = \textbf{\texttt{/dev/ttyAMA0}} default serial port on Raspberry Pi.\\
Baudrate = \textbf{38400} proved to work well with our implementation.\\
Bits = use \textbf{8} bits.\\
Stopbits = use \textbf{1}.\\
Data correction or Parity = set to \textbf{NO\_PARITY}.\\
...
\subsection{Gradle}
Gradle\footnote{\url{http://gradle.org/}} is a build automation tool and simplifies the build process drastically.
...
Although it looks like a configuration file, it is actually fully functional code written in Groovy\footnote{\url{http://www.groovy-lang.org/}}, a scripting language based on Java.
...
There is the opendial subproject, which is a copy of the dialog manager OpenDial\footnote{\url{http://www.opendial-toolkit.net/}}.
...
TestNG\footnote{\url{http://testng.org/doc/}} was used as testing framework in this project.
